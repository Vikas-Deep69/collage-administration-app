...
class AddAssignment:
    def __init__(self, frame):
        ...
        self.subject_entry=ttk.Combobox(self.frame1,width=17,values=database.getSubjectName(),font=("JUST Sans",11))
    ...
    def register_assignment(self):
        if self.course_entry.get() and self.department_entry.get() and self.subject_entry.get() and self.Number_entry.get() :
            res = databaseU.registerAssignment((self.course_entry.get(),self.department_entry.get(),self.subject_entry.get(),self.Number_entry.get()))
            if res:
