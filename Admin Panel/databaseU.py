...
def getbytopic(id):
    try:
        ...
        return []
#update
def updateMaterial(id,new_course,new_department,new_subject,new_topic):
    try:
        query = "UPDATE `add_material` SET Course = %s, Department = %s, Subject = %s,Topic = %s WHERE id = %s"
        data=(new_course,new_department,new_subject,new_topic,id[0])
...
def getbyassignment(id):
    try:
        ...
        return []
#update
def updateAssignment(id,new_course,new_department,new_subject,new_number):
    try:
        query = "UPDATE `add_assignment` SET course = %s, department = %s, subject = %s,number = %s WHERE id = %s"
        data=(new_course,new_department,new_subject,new_number,id[0])
        cursor.execute(query,data)
        Info.commit()
        return True
    except Exception as e:
